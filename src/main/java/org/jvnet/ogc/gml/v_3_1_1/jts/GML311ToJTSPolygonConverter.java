      ...
      if (abstractRingType instanceof LinearRingType) {
        ...
        throw new ConversionFailedException(
            locator.property("exterior", polygonType.getExterior())
                .property("value", polygonType.getExterior().getValue())
                .property("ring", polygonType.getExterior().getValue().getRing()), "Only linear rings are supported."); //$NON-NLS-1$
      }
    } else {
      shell = null;
    }
        ...
        if (abstractRingType instanceof LinearRingType) {
          holesList.add(linearRingConverter.createGeometry(
              entryLocator
                  ...
                  .property("ring", ringElement.getValue().getRing())
                  .property("value", ringElement.getValue().getRing().getValue()), (LinearRingType) abstractRingType));
        } else {
