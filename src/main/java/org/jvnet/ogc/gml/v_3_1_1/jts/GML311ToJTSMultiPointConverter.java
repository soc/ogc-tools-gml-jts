...
public class GML311ToJTSMultiPointConverter
  ...
  protected MultiPoint doCreateGeometry(ObjectLocator locator,
    ...
    if (multiPointType.isSetPointMember() || multiPointType.isSetPointMembers()) {
      return getGeometryFactory().createMultiPoint(points.toArray(new Point[0]));
    } else {
      throw new ConversionFailedException(locator, "Either [pointMember] or [pointMembers] elements are expected."); //$NON-NLS-1$
    }
  }

  public MultiPoint createGeometry(ObjectLocator locator, MultiPointPropertyType multiPointPropertyType)
      throws ConversionFailedException {
    if (multiPointPropertyType.isSetMultiPoint()) {
      return createGeometry(
          locator.property("multiPoint", multiPointPropertyType.getMultiPoint()), multiPointPropertyType.getMultiPoint()); //$NON-NLS-1$
    } else {
